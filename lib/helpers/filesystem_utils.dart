...
final String appDirectory = storageRootPath + 'SoruSkala';

final String rootFolders = appDirectory + '/Folders';
final String rootPdfs = appDirectory + '/Pdfs';
final String rootTags = appDirectory + '/Tags';
...
Future<List<Directory>> getStorageList() async {
  ...
  for (Directory dir in paths) {
    filteredPaths.add(await getExternalStorageWithoutDataDir(dir.absolute.path));
  }
  return filteredPaths;
}

Future<void> createAppRootPath() async {
  new Directory(appDirectory).create(recursive: false).then((value) => {print('create folders ===> success')}).catchError((onError){print('error ===> ' + onError.toString());});
  new Directory(rootFolders).create(recursive: false).then((value) => {print('create folders ===> success')}).catchError((onError){print('error ===> ' + onError.toString());});
  new Directory(rootPdfs).create(recursive: false).then((value) => {print('create pdfs ===> success')}).catchError((onError){print('error ===> ' + onError.toString());});
  new Directory(rootTags).create(recursive: false).then((value) => {print('create tags ===> success')}).catchError((onError){print('error ===> ' + onError.toString());});
}
...
Future<Directory> getExternalStorageWithoutDataDir(String unfilteredPath) async {
  PackageInfo packageInfo = await PackageInfo.fromPlatform();
  print("storage_helper->getExternalStorageWithoutDataDir: " + packageInfo.packageName);
  String subPath = pathlib.join("Android", "data", packageInfo.packageName, "files");
...
Future<List<FileSystemEntity>> getFoldersAndFiles(String path, {changeCurrentPath: true,
    Sorting sortedBy: Sorting.Type, reverse: false, recursive: false, keepHidden: false}) async {
...
/// keepHidden: show files that start with .
Stream<List<FileSystemEntity>> fileStream(String path, {changeCurrentPath: true,
    Sorting sortedBy: Sorting.Type, reverse: false, recursive: false, keepHidden: false}) async* {
    ...
    if (_path.listSync(recursive: recursive).length != 0) {
      if (!keepHidden) {
        yield* _path.list(recursive: recursive).transform(
            StreamTransformer.fromHandlers(handleData: (FileSystemEntity data, sink) {
...
Future<List<FileSystemEntity>> search(dynamic path, String query,
  ...
  List<FileSystemEntity> files = await getFoldersAndFiles(path, recursive: recursive, keepHidden: hidden)
    ..retainWhere((test) => test.basename().toLowerCase().contains(query.toLowerCase()));
